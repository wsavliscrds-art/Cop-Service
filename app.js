...
function buildOverviewSkeleton() {
  return `
    <div class="overview-top-grid">
      ...
      <section class="overview-metrics" id="overview-metrics"></section>
    </div>
    ...
    <div class="overview-bottom-grid">
      ...
      <section class="panel quick-access"><div class="panel-header"><div class="panel-title">Acesso rápido</div></div><div class="quick-grid"><button class="quick-card" data-view-nav="catalog"><span>▦</span><strong>Catálogo de serviços</strong><small>Explore e solicite serviços</small><b>→</b></button><button class="quick-card" data-view-nav="assets"><span>◫</span><strong>Meus ativos</strong><small>Consulte seus equipamentos</small><b>→</b></button><button class="quick-card" data-view-nav="tickets"><span>◌</span><strong>Meus chamados</strong><small>Acompanhe solicitações</small><b>→</b></button><button class="quick-card" data-view-nav="approvals"><span>✓</span><strong>Aprovações</strong><small>Veja pendências</small><b>→</b></button></div></section>
    </div>
  `;
}
...
function renderOverview() {
  ...
  document.getElementById('overview-metrics').innerHTML = metricData.map((m, i) => '<article class="metric-card metric-card-' + i + '"><span class="metric-icon">' + ['▣', '◉', '◷', '✓'][i] + '</span><span class="metric-label">' + escapeHtml(m[0]) + '</span><strong>' + m[1] + '</strong><small>' + escapeHtml(m[2]) + '</small><a data-view-nav="' + (i === 2 ? 'approvals' : i === 1 ? 'tickets' : 'atendimentos') + '">Ver todos →</a></article>').join('');
  const filtered = open.filter((t) => !overviewPriority || t.priority === overviewPriority).sort((a, b) => agingMs(b) - agingMs(a));
  document.getElementById('overview-open-list').innerHTML = filtered.length ? filtered.slice(0, 5).map((t) => '<div class="list-row" data-ticket="' + t.id + '"><div class="lr-left"><div class="lr-icon" style="' + catChipStyle(t.category) + '">' + catIconOr(t.category) + '</div><div><div class="lr-title">' + escapeHtml(t.title || t.service) + ' <span class="mono">' + t.id + '</span></div><div class="lr-meta">' + escapeHtml(categoryLabel(t.category)) + ' · ' + escapeHtml(currentHandler(t)) + '</div></div></div><div class="lr-right">' + priorityBadge(t.priority) + '<span class="row-age">' + formatDuration(agingMs(t)) + '</span></div></div>').join('') : '<div class="overview-empty"><span>▱</span><strong>Nenhum chamado em aberto.</strong><small>Tudo certo! Não há chamados em atendimento agora.</small><button class="btn btn-secondary btn-sm" data-open-new-ticket>Abrir novo chamado</button></div>';
